...
import { StaticTriggeredAbilityModel } from "~/engine/store/models/StaticTriggeredAbilityModel";
import { DelayedTriggeredAbilityModel } from "~/engine/store/models/DelayedTriggeredAbilityModel";
  ...
  trigger(trigger: OnTrigger, card: CardModel) {
    ...
    triggers.forEach((ability) => {
      ...
      switch (trigger.on) {
        ...
        case "quest": {
          if (card.instanceId === ability.source.instanceId) {
            this.rootStore.stackLayerStore.addAbilityToStack(
              layer,
              ability.source,
            );
