...
describe("Scar Shameless Firebrand", () => {
    it("ROUSING SPEECH effect- Ready All characters with cost 3 or less", () => {
        ...
        const cardUnderTest = testStore.getByZoneAndId("hand", scarShamelessFirebrand.id);
        ...
        const threeCostTarget = testStore.getByZoneAndId("play", drFacilierCharlatan.id);
        ...
        expect(testStore.getByZoneAndId("play", drFacilierCharlatan.id).meta).toEqual(
            expect.objectContaining({ exerted: false })
        );
    });
